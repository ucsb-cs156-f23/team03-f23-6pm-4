...
describe("UserTable tests", () => {
  ...
  test("Has the expected column headers and content for ordinary user", () => {
    ...
    render(
      <QueryClientProvider client={queryClient}>
        <MemoryRouter>
          <HelpRequestTable requests={helpRequestsFixtures.threeRequests} currentUser={currentUser} />
        </MemoryRouter>
      </QueryClientProvider>
  ...
  test("Delete button calls delete callback", async () => {
    ...
    await waitFor(() => { expect(screen.getByTestId(`HelpRequestTable-cell-row-0-col-id`)).toHaveTextContent("1"); });
    await waitFor(() => { expect(screen.getByTestId(`HelpRequestTable-cell-row-0-col-solved`)).toHaveTextContent("false"); });
