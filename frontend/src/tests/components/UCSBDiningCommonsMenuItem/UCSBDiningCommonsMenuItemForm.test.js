...
describe("UCSBDiningCommonsMenuItemForm tests", () => {
    ...
    test("Correct Error messsages on bad input", async () => {
        ...
        await screen.findByText(/diningCommonsCode must be numeric, e.g. 100/);
    });
    ...
    test("No Error messsages on good input", async () => {
        ...
        fireEvent.change(diningCommonsCodeField, { target: { value: '100' } });
        fireEvent.change(nameField, { target: { value: 'Taco' } });
        fireEvent.change(stationField, { target: { value: 'Mexican' } });
        fireEvent.click(submitButton);

        await waitFor(() => expect(mockSubmitAction).toHaveBeenCalled());

        expect(screen.queryByText(/diningCommonsCode must be numeric, e.g. 100/)).not.toBeInTheDocument();
        expect(screen.queryByText(/Name must be alphabetical, e.g. Taco/)).not.toBeInTheDocument();
        expect(screen.queryByText(/Station must be alphabetical, e.g. Entree/)).not.toBeInTheDocument();
    });
