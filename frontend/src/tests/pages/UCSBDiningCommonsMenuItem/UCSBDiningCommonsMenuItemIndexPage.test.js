...
describe("UCSBDiningCommonsMenuItemIndexPage tests", () => {
    ...
    test("renders three menu items correctly for regular user", async () => {
        ...
        const diningCommonsCode = screen.getByText("100");
        expect(diningCommonsCode).toBeInTheDocument();

        const name = screen.getByText("Taco");
        expect(name).toBeInTheDocument();

        const station = screen.getByText("Mexican");
    ...
    test("what happens when you click delete, admin", async () => {
        ...
        axiosMock.onGet("/api/ucsbdiningcommonsmenuitems/all").reply(200, ucsbDiningCommonsMenuItemFixtures.threeMenuItems);
        axiosMock.onDelete("/api/ucsbdiningcommonsmenuitems").reply(200, "Menu Item with id 1 was deleted");
