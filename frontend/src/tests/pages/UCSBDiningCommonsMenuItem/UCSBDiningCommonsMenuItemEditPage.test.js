...
describe("UCSBDiningCommonsMenuItemEditPage tests", () => {
    ...
    describe("tests where backend is working normally", () => {
        ...
        test("Is populated with the data provided", async () => {
            ...
            expect(diningCommonsCodeField).toHaveValue("100");
            expect(nameField).toBeInTheDocument();
            expect(nameField).toHaveValue("Taco");
            expect(stationField).toBeInTheDocument();
            expect(stationField).toHaveValue("Mexican");

            expect(submitButton).toHaveTextContent("Update");

            fireEvent.change(diningCommonsCodeField, { target: { value: '200' } });
            fireEvent.change(nameField, { target: { value: 'Steak Taco' } });
            fireEvent.change(stationField, { target: { value: 'Entrees' } });
            fireEvent.click(submitButton);

            await waitFor(() => expect(mockToast).toBeCalled());
            expect(mockToast).toBeCalledWith("UCSBDiningCommonsMenuItem Updated - id: 17 name: Steak Taco");
        ...
        test("Changes when you click Update", async () => {
            ...
            const idField = screen.getByTestId("UCSBDiningCommonsMenuItemForm-id");
            const diningCommonsField = screen.getByTestId("UCSBDiningCommonsMenuItemForm-diningCommonsCode");
            const nameField = screen.getByTestId("UCSBDiningCommonsMenuItemForm-name");
            const stationField = screen.getByTestId("UCSBDiningCommonsMenuItemForm-station");
            const submitButton = screen.getByTestId("UCSBDiningCommonsMenuItemForm-submit");

            expect(idField).toHaveValue("17");
            expect(diningCommonsField).toHaveValue("100");
            expect(nameField).toHaveValue("Taco");
            expect(stationField).toHaveValue("Mexican");
            expect(submitButton).toBeInTheDocument();

            fireEvent.change(diningCommonsField, { target: { value: '200' } });
            fireEvent.change(nameField, { target: { value: 'Steak Taco' } })
            fireEvent.change(stationField, { target: { value: 'Entrees' } })
